 ...
 * $Revision: 3 $
 * $Date: 15/05/12 1:35p $
 * @brief    NUC230_240 series Smartcard UART mode (SCUART) driver source file
 *
 * @note
...
static uint32_t SCUART_GetClock(SC_T *sc)
{
    ...
    u32ClkSrc = (CLK->CLKSEL3 >> (2 * u32Num)) & CLK_CLKSEL3_SC0_S_Msk;
    ...
    u32Clk /= (((CLK->CLKDIV1 >> (8 * u32Num)) & CLK_CLKDIV1_SC0_N_Msk) + 1);
